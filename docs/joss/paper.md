...
where $I_t$ is the inventory level at period $t$. The higher the holding cost, the more costly it is to keep the inventory (when the inventory level is positive). The higher the shortage cost, the more costly it is to run out of stock (when the inventory level is negative). The single-period, joint holding and stockout cost can be can be calculated using the `get_cost` method of the sourcing model.
